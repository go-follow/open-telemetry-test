...
func NewZapToGokitLogAdapter(logger *zap.Logger, opts ...Options) gokitLog.Logger {
	// need to skip two levels in order to get the correct caller
	// one for this method, the other for gokitLog
	logger = logger.WithOptions(zap.AddCallerSkip(2))

	options := OptionsScrape{}
	for _, opt := range opts {
		opt(&options)
	}
	return &zapToGokitLogAdapter{l: logger.Sugar(), scrape: scrapeConfig{
		prometheusErrCh: options.ScrapePrometheusErrCh,
		filterErrMsg:    options.FilterErrMsg,
	}}
}
...
func (w *zapToGokitLogAdapter) Log(keyvals ...any) error {
	...
	if len(keyvals)%2 == 0 {
		...
		go func() {
			if w.scrape.prometheusErrCh != nil && w.scrape.filterErrMsg == ld.msg && ld.level == level.DebugValue() {
				w.scrape.prometheusErrCh <- ld.err
			}
		}()
